...
```bash
./gradlew :template-backend:check
./gradlew :template-backend:securityScan
```
...
- `backend` built from `template-backend/Dockerfile`
...
./gradlew :template-backend:bootRunMysql
```
